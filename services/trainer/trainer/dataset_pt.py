...
class DraftSequenceDataset(Dataset):
    ...
    def __init__(self, heroes_seqs, actions_seqs, tabular_feats, patches, labels, max_len=25):
        n = len(labels)
    ...
    def __getitem__(self, idx):
        return self.heroes[idx], self.actions[idx], self.tabular[idx], self.patches[idx], self.labels[idx]
...
def load_sequence_dataset(cfg: TrainerConfig, engine, max_len: int = 25):
    ...
    # Use fast SQL with aggregate tables (no LATERAL joins), with lookback
    sql = training_features_sql_fast(_match_extra_where(cfg), lookback=cfg.lookback_patches)
    logger.info("Reading training data from aggregate tables (lookback=%d)...", cfg.lookback_patches)
    import time
    t0 = time.time()
    from sqlalchemy import text
    with engine.connect() as conn:
        result = conn.execute(text(sql), {"patch_id": cfg.patch_id, "lookback": cfg.lookback_patches})
        columns = result.keys()
    ...
    if df.empty:
        raise ValueError(f"No training data found for patch {cfg.patch_id} (lookback={cfg.lookback_patches}).")
    ...
    t0 = time.time()
    t_h, t_a, t_t, t_p, t_l = _build_augmented_data(train_df, agg_cols)
    v_h, v_a, v_t, v_p, v_l = _build_augmented_data(val_df, agg_cols)
    logger.info("Augmentation done in %.0fs — train: %d seqs, val: %d seqs",
                time.time() - t0, len(t_l), len(v_l))

    train_ds = DraftSequenceDataset(t_h, t_a, t_t, t_p, t_l, max_len)
    val_ds = DraftSequenceDataset(v_h, v_a, v_t, v_p, v_l, max_len)
