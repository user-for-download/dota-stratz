...
- `internal/config/` — YAML + env expansion. Parses `FETCH_LOBBY_TYPES` (comma-separated) into `[]int`. `watermark_lookback_days` defaults to `fetch_last_count_day` at runtime (not a static 30) so it never fails validation. Validates required fields: `RabbitMQ.URL`, `RabbitMQ.Queues.MatchIDs`, `Redis.Addr`. New fields: `start_run`, `start_run_min_pool_size`, `start_run_max_wait`.
...
- `internal/consumer/` — RabbitMQ consumer for `queue.match_ids` with DLQ binding and QoS. Auto-reconnection loop (1s → 30s exponential backoff with context-aware sleep).
...
- `internal/config/` — YAML + env expansion. Validates required fields: `RabbitMQ.URL`, `RabbitMQ.Queues.MatchIDs`, `RabbitMQ.Queues.RawMatches`, `Postgres.DSN`.
...
**Packages:**
- `internal/consumer/` — RabbitMQ consumer for `queue.raw_matches` with `ConsumeWithReconnect` (auto-reconnection with context-aware backoff), `NewDLQChannel()` for independent DLQ publishing
- `internal/config/` — YAML + env expansion. Validates required fields: `Postgres.DSN`, `RabbitMQ.URL`, `RabbitMQ.Queues.RawMatches`.
...
- `internal/repository/batch_writer.go` — Batch insert into 19+ tables per match in a single `pgx.Batch` transaction with `ON CONFLICT DO NOTHING` (idempotency). All child table helpers return errors. Uses `context.WithoutCancel` for I/O to prevent connection corruption on graceful shutdown. Rollback uses a 10-second timeout context to prevent indefinite blocking on unreachable DB.
...
- `internal/validator/` — Concurrent proxy validator (150 workers default). `ValidateStream()` uses `proxypool.MakeTransport` for HTTP transport (now supports HTTP, HTTPS, SOCKS4, and SOCKS5 proxy schemes). Performs GET, drains 256 bytes on success, drains body on non-200 status to prevent resource leaks. Reports progress every 30s. Guaranteed goroutine cleanup via `defer close(progressDone)`.
...
3. **Feature extraction** — `TRAINING_FEATURES_SQL` computes **59 aggregate + 160 one-hot hero ID = 219-dim feature vectors** via a single query with `LATERAL` subqueries. Includes:
   ...
   - Hero draft-slot (pick-position) win rates
   - Low-game missingness flags, delta features, role interactions
4. **Training** — LightGBM **binary classification** with early stopping (50 rounds), 85/15 chronological train/val split. **Platt scaling calibration** via LogisticRegression on validation set only (single fit to avoid overfitting).
5. **Output** — Model (`model_patch_N.txt`), calibrator (`calibrator_patch_N.json`), metadata, and `feature_schema_patch_N.json` (219-column contract).
6. **Validation** — Patch 60 achieves: **binary_logloss 0.6894** with 219 features. Future improvements planned: recent form, meta drift, sequence context, and team strategy features.

**Key behaviors:**
- Idempotent: aggregates use INSERT ON CONFLICT DO UPDATE
- Feature column order frozen in `feature_schema.json` (219 columns); API loads this to guarantee agreement
- Platt scaling calibration: raw LightGBM scores → calibrated 0-1 probabilities (fit on validation set only)
...
- Return type annotation matches actual return value (5-tuple: train_data, val_data, metadata, X_train, y_train)
...
4. For each candidate hero not yet picked/banned, builds **219-dim feature vector** (59 aggregate + 160 one-hot hero ID) and scores via LightGBM
5. Applies Platt scaling calibration for calibrated win probabilities
6. Re-ranks using 1-ply look-ahead minimax search (uses actual draft state and correct team IDs)
7. Returns top-N recommendations with calibrated `pick_probability` and `win_probability`

**Feature categories (219 total):**
...
- Model loading: file I/O (model/schema/calibrator) happens outside the lock to avoid blocking all threads during lazy load
- Feature computation mirrors trainer logic exactly (same 219-column order via `feature_schema.json`)
- NULL-safe: all aggregate lookups guarded by `_float()`/`_int()` helpers; `.get()` defaults use `or 0` to handle NULL DB values
...
- Look-ahead uses actual draft state and correct `recommending_team` for opponent simulation

---

### 7. Frontend (Draft Predictor)

**Purpose:** Single-page web application for interactive draft prediction. Users select teams, choose first-pick side, and get real-time AI recommendations during the draft.

| Aspect | Detail |
|---|---|
| Package | `services/frontend/` |
| Port | 80 (Nginx) |
| Dependencies | ML API (reverse proxy `/api/*` → `dota2-ml-api:8080`) |
| Stack | Vanilla HTML/CSS/JS (no build step), Nginx, Docker (nginx:alpine + curl for healthcheck) |

**Features:**
- **Team selection:** 1000 teams from `teams.json` with mutual exclusion (same team can't be both Radiant and Dire)
- **First pick selector:** Toggle Radiant/Dire first pick, which changes the draft order pattern
- **Draft flow:** 24-slot Captain's Mode (ban/pick alternating), with Start Draft button
- **Recommendation panels:** Dual-column layout showing bans and picks separately
  - Bans phase: Dire LEFT, Radiant RIGHT
  - Picks phase: Radiant LEFT, Dire RIGHT
  - Active team's chips are clickable; inactive team's chips are grayed out
- **Team-hero boost:** Heroes with 65%+ WR and 3+ games get score boosts, displayed with ★ tag
- **Hero highlights:** Top 5 recommended heroes highlighted in grid with colored borders and badges
- **Tooltip predictions:** 300ms debounced hover shows WR, pick probability, team record, rank
- **Hero images:** 127 local icon images (~1MB total) from Steam CDN, no external loading

**API contract:**
- `POST /predict` with `for_team` parameter (0 or 1) for per-team recommendations
- `num_recommendations` up to 50
- Empty draft array supported for initial predictions (before first action)
- Response includes `team_games`, `team_win_rate`, `boosted` fields for team proficiency

**Architecture:**
```
Browser ──► Nginx (:80)
              ├── /          → index.html (static)
              ├── /img/      → hero icon PNGs (static)
              └── /api/*     → dota2-ml-api:8080 (reverse proxy)
```
...
| `mq` | `rabbitmq.go`, `queue.go`, `consumer.go`, `publisher.go` | `Connect(url)`, `QueueConfig`, `DeclareQueueWithDLQ(ch, cfg)`, `Consumer`, `Publisher` | AMQP connection + channel. Extended with shared queue declaration (`DeclareQueueWithDLQ`), auto-reconnecting `Consumer` with `ConsumeWithReconnect` (context-aware backoff — respects `ctx.Done()` during reconnect sleep to prevent delayed shutdown), and `Publisher` with publisher confirms + automatic reconnect. Previously each service duplicated reconnection logic and queue declaration; these are now centralized in the shared package. |
...
- Failure classification: `HardFailure`/`BadStatus` → permanent removal, `RateLimited` → cooldown (with ZScore check to prevent re-adding proxies removed by concurrent `remove()`), `Timeout` → incremental counter → removal at threshold (3)
...
- **`Consumer`** — Wraps a RabbitMQ connection with automatic reconnection. `ConsumeWithReconnect(done, tag)` returns a channel that survives broker restarts (exponential backoff 1s → 30s). The channel is never closed on reconnect — only on permanent shutdown via the `done` channel. Backoff sleeps use `select` on `ctx.Done()` to prevent delayed shutdown during reconnect.
