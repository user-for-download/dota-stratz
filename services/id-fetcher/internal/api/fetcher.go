...
func (f *Fetcher) Run(ctx context.Context) error {
	...
	for _, m := range matches {
		...
		} else if !f.forceDownload {
			// Fallback: use Redis lastMaxMatchID to avoid publishing duplicate matches
			// when Postgres is unreachable and forceDownload is false.
			if m.MatchID <= f.lastMaxMatchID {
				continue
			}
		}
