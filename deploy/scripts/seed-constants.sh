...
# ── Heroes ──────────────────────────────────────────────────────────────────
if ! curl -sf 'https://api.opendota.com/api/heroes' -o /tmp/heroes.json; then
    echo "ERROR: Failed to fetch heroes from OpenDota API"
    exit 1
fi
...
# ── Items ───────────────────────────────────────────────────────────────────
if ! curl -sf 'https://api.opendota.com/api/constants/items' -o /tmp/items.json; then
    echo "ERROR: Failed to fetch items from OpenDota API"
    exit 1
fi
...
# ── Abilities ───────────────────────────────────────────────────────────────
if ! curl -sf 'https://api.opendota.com/api/constants/ability_ids' -o /tmp/ability_ids.json; then
    echo "ERROR: Failed to fetch ability_ids from OpenDota API"
    exit 1
fi
if ! curl -sf 'https://api.opendota.com/api/constants/abilities' -o /tmp/abilities.json; then
    echo "ERROR: Failed to fetch abilities from OpenDota API"
    exit 1
fi
...
# ── Item IDs ────────────────────────────────────────────────────────────────
if ! curl -sf 'https://api.opendota.com/api/constants/item_ids' -o /tmp/item_ids.json; then
    echo "ERROR: Failed to fetch item_ids from OpenDota API"
    exit 1
fi
...
if ! curl -sf 'https://api.opendota.com/api/constants/hero_abilities' -o /tmp/hero_abilities.json; then
    echo "ERROR: Failed to fetch hero_abilities from OpenDota API"
    exit 1
fi
