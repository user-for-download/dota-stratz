...
func (p *Pool) cooldown(ctx context.Context, proxy string) error {
	// Check if the proxy still exists in the pool before re-adding.
	// A concurrent remove() may have already evicted it.
	_, err := p.rdb.ZScore(ctx, RedisKey, proxy).Result()
	if err != nil && err != redis.Nil {
		return err
	}
	if err == redis.Nil {
		// Proxy was already removed — don't re-add it.
		logger.Log.Debug("Cooldown skipped: proxy already removed",
			zap.String("proxy", proxy))
		return nil
	}
