...
func runValidation(
	...
	// Circuit breaker: if >80% of proxies failed, OpenDota may be down or
	// rate-limiting. Skip adding new proxies to prevent mass pool wipeout.
	if stats.Total > 10 && stats.SuccessRate() < 0.20 {
		logger.Log.Warn("Validation circuit breaker triggered — skipping pool update",
