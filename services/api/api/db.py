...
def get_conn() -> Any:
    ...
    try:
        # Check pool liveness under the lock so close_pool() cannot race
        # between the check and getconn().
        with _pool_lock:
            if _pool is None:
                raise RuntimeError("DB pool closed before get_conn")
        # getconn() runs OUTSIDE the lock so a concurrent put_conn() can
        # return connections even when the pool is exhausted (BUG-004
        # deadlock fix). The semaphore guarantees at most pool_max callers
        # reach this point, so getconn() on a healthy pool returns
        # immediately from the free list.
