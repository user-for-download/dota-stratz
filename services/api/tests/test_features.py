...
class TestBuildFeatureVector:
    ...
    numeric aggregate columns + hero_id categorical + 32-D embeddings.

    The output shape must be ``(len(aggregate_columns) + 1 + n_embeddings,)``
    and individual column values must match the contract established by
    the trainer's ``training_features_sql`` query contract.
    """
    ...
    def test_output_shape(self, feature_schema, batch_context_factory,
        ...
        """Output shape = (len(agg_cols) + 1 (hero_id) + n_embeddings)."""
        schema = {**feature_schema, "n_embeddings": N_EMBEDDINGS}
        ...
        expected = len(schema["aggregate_columns"]) + 1 + N_EMBEDDINGS
        assert fv.shape == (expected,), (
            f"Expected shape ({expected},), got {fv.shape}"
        )

    def test_hero_id_categorical_position(self, feature_schema,
                                          batch_context_factory,
                                          draft_context_factory):
        """hero_id is placed at index len(agg_cols) with correct float value."""
        schema = {**feature_schema, "n_embeddings": N_EMBEDDINGS}
        batch = batch_context_factory()
        ctx = draft_context_factory()
        hero_id = 42
        ...
        assert fv[n_agg] == 42.0, f"Expected hero_id=42.0 at index {n_agg}"

    def test_embeddings_correct_position(self, feature_schema,
                                         batch_context_factory,
                                         draft_context_factory):
        """32-D embedding vector is placed after hero_id."""
        schema = {**feature_schema, "n_embeddings": N_EMBEDDINGS}
        batch = batch_context_factory()
        ctx = draft_context_factory()
        hero_id = 1
        mock_emb = {str(hero_id): [0.5] * N_EMBEDDINGS}
        fv = build_feature_vector(
            hero_id=hero_id, ctx=ctx, patch_id=1, batch=batch, schema=schema,
            embeddings=mock_emb,
        )
        n_agg = len(schema["aggregate_columns"])
        embs = fv[n_agg + 1:]  # Skip hero_id column
        assert np.all(embs == 0.5), "Embedding vector not correctly placed"

    def test_unknown_hero_gets_zero_embeddings(self, feature_schema,
                                               batch_context_factory,
                                               draft_context_factory):
        """hero not in embeddings dict → all-zero embedding vector."""
        schema = {**feature_schema, "n_embeddings": N_EMBEDDINGS}
        batch = batch_context_factory()
        ctx = draft_context_factory()
        fv = build_feature_vector(
            hero_id=999, ctx=ctx, patch_id=1, batch=batch, schema=schema,
            embeddings={"1": [0.9] * N_EMBEDDINGS},  # hero 999 not present
        )
        n_agg = len(schema["aggregate_columns"])
        embs = fv[n_agg + 1:]
        assert float(embs.sum()) == 0.0
    ...
    def test_hds_games_is_integer_valued_float(self, feature_schema,
                                               batch_context_factory,
                                               draft_context_factory):
        """hds_games is integer-valued (3.0, not 3.14) — REGRESSION BUG-013."""
        schema = {**feature_schema, "n_embeddings": N_EMBEDDINGS}
        batch = batch_context_factory(
    ...
    def test_ph_is_new_player_when_games_below_5(
        ...
        batch = batch_context_factory(player_hero_agg={1: {"games": 3}})
        fv = build_feature_vector(hero_id=1, ctx=ctx, patch_id=1, batch=batch, schema=schema, embeddings={})
        assert fv[idx] == 1.0

        batch = batch_context_factory(player_hero_agg={1: {"games": 5}})
        fv = build_feature_vector(hero_id=1, ctx=ctx, patch_id=1, batch=batch, schema=schema, embeddings={})
        assert fv[idx] == 0.0
    ...
    def test_th_is_new_team_hero_when_games_below_5(
        ...
        batch = batch_context_factory(team_hero_agg={1: {"games": 2}})
        fv = build_feature_vector(hero_id=1, ctx=ctx, patch_id=1, batch=batch, schema=schema, embeddings={})
        assert fv[idx] == 1.0

        batch = batch_context_factory(team_hero_agg={1: {"games": 5}})
        fv = build_feature_vector(hero_id=1, ctx=ctx, patch_id=1, batch=batch, schema=schema, embeddings={})
        assert fv[idx] == 0.0
    ...
    def test_ph_vision_support_score_non_zero_only_when_lane_role_5(
            self, feature_schema, batch_context_factory,
            draft_context_factory):
        """ph_vision_support_score non-zero only when ph_lane_role == 5."""
        schema = {**feature_schema, "n_embeddings": N_EMBEDDINGS}
        ctx = draft_context_factory()
        idx = schema["aggregate_columns"].index("ph_vision_support_score")

        batch = batch_context_factory(player_hero_agg={1: {"lane_role": 5, "avg_vision_placed": 42.0}})
        fv = build_feature_vector(hero_id=1, ctx=ctx, patch_id=1, batch=batch, schema=schema, embeddings={})
        assert fv[idx] == 42.0

        batch = batch_context_factory(player_hero_agg={1: {"lane_role": 1, "avg_vision_placed": 42.0}})
        fv = build_feature_vector(hero_id=1, ctx=ctx, patch_id=1, batch=batch, schema=schema, embeddings={})
        assert fv[idx] == 0.0
    ...
    def test_ph_gpm_carry_score_non_zero_only_when_lane_role_1(
            self, feature_schema, batch_context_factory,
            draft_context_factory):
        """ph_gpm_carry_score non-zero only when ph_lane_role == 1."""
        schema = {**feature_schema, "n_embeddings": N_EMBEDDINGS}
        ctx = draft_context_factory()
        idx = schema["aggregate_columns"].index("ph_gpm_carry_score")

        batch = batch_context_factory(player_hero_agg={1: {"lane_role": 1, "avg_gpm": 650.0}})
        fv = build_feature_vector(hero_id=1, ctx=ctx, patch_id=1, batch=batch, schema=schema, embeddings={})
        assert fv[idx] == 650.0

        batch = batch_context_factory(player_hero_agg={1: {"lane_role": 5, "avg_gpm": 650.0}})
        fv = build_feature_vector(hero_id=1, ctx=ctx, patch_id=1, batch=batch, schema=schema, embeddings={})
        assert fv[idx] == 0.0
...
class TestPreFetchBatch:
    ...
    def test_team_id_none_returns_empty_team_agg(self):
        with mock.patch("api.features.db_") as mock_db:
            ...
            result = pre_fetch_batch(patch_id=1, hero_ids=[1, 2], team_id=None, enemy_team_id=None, ctx=None, account_id=None)
    ...
    def test_team_id_zero_triggers_query(self):
        with mock.patch("api.features.db_") as mock_db:
            ...
            result = pre_fetch_batch(patch_id=1, hero_ids=[1, 2], team_id=0, enemy_team_id=None, ctx=None, account_id=None)
    ...
    def test_account_id_zero_executes_query(self):
        with mock.patch("api.features.db_") as mock_db:
            ...
            result = pre_fetch_batch(patch_id=1, hero_ids=[1, 2], team_id=None, enemy_team_id=None, ctx=None, account_id=0)
            mock_db.fetch_player_hero_agg_batch.assert_called_once_with(1, 0, [1, 2])
    ...
    def test_team_pick_ordinal_from_ally_picks(self):
        ctx = DraftContext(turn=3, recommending_team=0, is_pick_turn=True, radiant_picks=[1, 5])
        with mock.patch("api.features.db_") as mock_db:
            mock_db.fetch_baselines_batch.return_value = {}
            mock_db.fetch_hero_draft_slot_batch.return_value = {}
            result = pre_fetch_batch(patch_id=1, hero_ids=[1, 2], team_id=None, enemy_team_id=None, ctx=ctx, account_id=None)
            mock_db.fetch_hero_draft_slot_batch.assert_called_once_with(1, [1, 2], 3)

    def test_no_hero_ids_returns_empty_baselines(self):
        with mock.patch("api.features.db_") as mock_db:
            mock_db.fetch_baselines_batch.return_value = {}
            result = pre_fetch_batch(patch_id=1, hero_ids=[], team_id=None, enemy_team_id=None, ctx=None, account_id=None)
